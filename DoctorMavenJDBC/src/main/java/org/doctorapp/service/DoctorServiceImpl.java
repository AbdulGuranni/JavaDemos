...
public class DoctorServiceImpl implements IDoctorService {
    ...
    public List<Doctor> getBySpecialityAndExp(String speciality, int experience)throws DoctorNotFoundException {
        ...
        if(doctorList.isEmpty())
            throw  new DoctorNotFoundException("Doctor with this speciality and experience is not available");
        Collections.sort(doctorList,(d1,d2)->((Integer)(d2.getExperience())).compareTo(d1.getDoctorId()));
        return doctorList;
    }

    @Override
    public List<Doctor> getBySpecialityAndLesFess(String speciality, double fees)throws DoctorNotFoundException {
        List<Doctor> doctorList = doctorRepository.findBySpecialityAndLesFess(speciality,fees);
        if(doctorList.isEmpty())
            throw  new DoctorNotFoundException("Doctor with this speciality and fees is not available");
